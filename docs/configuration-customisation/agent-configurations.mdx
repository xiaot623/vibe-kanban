...
```json profiles.json
{
  "executors": {
    ...
    "GEMINI": {
      ...
      "PLAN":    { "GEMINI": { "model": "gemini-3-pro-preview", "plan": true, "yolo": false } },
...
<Tab title="GEMINI">
  ...
  <ParamField path="plan" type="boolean">
  Enable Gemini plan mode. If `plan` and `yolo` are both true, `plan` takes precedence.
  </ParamField>

  <ParamField path="yolo" type="boolean">
  Run without confirmations (ignored when `plan` is true)
  </ParamField>

  Plan mode requires a Gemini CLI version that supports `--approval-mode plan` (for example, `@google/gemini-cli@0.30.0` or newer).
